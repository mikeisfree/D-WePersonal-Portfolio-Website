...
const TAG_POOL = ['Identity', 'Editorial', 'Web', 'Type', 'Packaging', 'Direction', 'Motion', 'Print', 'System', 'Poster']
const CLIENTS = ['Atelier Borel', 'Maison Klein', 'Kunsthalle Bern', 'Volume Zero', 'Studio Forme', 'La Compagnie', 'Index Magazine', 'Foundry Y', 'Centre Marlot', 'Type West', 'Office of Things', 'Press Nord']
const ROLES = ['Lead Designer', 'Art Direction', 'Type & Layout', 'Identity Designer', 'Creative Direction']
const YEARS = [2026, 2025, 2025, 2024, 2024, 2023, 2023, 2022, 2022, 2021]
const TITLES = ['Counterpoint', 'Field Notes', 'Minor Index', 'Long Form', 'Sans Serif', 'Working Title', 'Volume One', 'Continued', 'Inventory', 'After Image', 'Marginal', 'Verso & Recto', 'Plain Text', 'Open Set', 'New Standard', 'Footnote', 'Annotated', 'Reprint']
...
function makeProjects() {
  return Array.from({ length: TILE_COUNT }, (_, i) => {
    const tags = [TAG_POOL[i % TAG_POOL.length], TAG_POOL[(i + 3) % TAG_POOL.length], TAG_POOL[(i + 7) % TAG_POOL.length]]
    return {
      id: i,
      num: String(i + 1).padStart(3, '0'),
      title: TITLES[i % TITLES.length] + ' №' + String((i % 12) + 1).padStart(2, '0'),
...
function placeholderColor(seed) {
  ...
  const l = 35 + rand(seed * 1.3) * 40
  const c = 0.04 + rand(seed * 2.7) * 0.08
  const h = rand(seed * 4.1) * 360
  return `oklch(${l.toFixed(1)}% ${c.toFixed(3)} ${h.toFixed(1)})`
}

// Defer all DOM work until after page load so initial GSAP animations are unblocked
window.addEventListener('load', () => {
  ...
  const tiles = projects.map((p, i) => {
    ...
    img.addEventListener('load', () => { a.dataset.loaded = 'true' }, { once: true })
    img.addEventListener('error', () => { a.dataset.loaded = 'true' }, { once: true })
  ...
  function fillDetail(p) {
    ...
    detailRelated.innerHTML = p.related.map((src) => `<img src="${src}" alt="" loading="lazy">`).join('')
  ...
  function openProject(idx) {
    ...
    if (document.startViewTransition) {
      ...
      t.finished.finally(() => { detailImg.style.viewTransitionName = '' })
    } else {
      applyOpenState()
    }
    detail.scrollTop = 0
  ...
  function closeProject() {
    if (!activeTile) { applyCloseState(); return }
    ...
    if (document.startViewTransition) {
      ...
      t.finished.finally(() => { tileImg.style.viewTransitionName = ''; activeTile = null })
    } else {
      applyCloseState()
      activeTile = null
    }
